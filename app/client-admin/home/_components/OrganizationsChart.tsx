...
function OrganizationsChart({
  organizationsChartData = [],
}: {
  organizationsChartData: AreaChatType[];
}) {
  ...
  useEffect(() => {
    if (!organizationsChartData.length) return;
    setChartData((pre) => {
      return pre.map((v) => {
        const monthValue = v.month.toLowerCase();
        const findValue = organizationsChartData.find(
